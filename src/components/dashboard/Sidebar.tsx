...
export default function Sidebar({ merchant, isOpen, setIsOpen }: any) {
  ...
  return (
    <aside className={`fixed top-[72px] bottom-0 left-0 z-40 w-72 bg-[#0B1120] border-r border-slate-800/80 transform transition-transform duration-300 ease-in-out flex flex-col ${showSidebar ? 'translate-x-0' : '-translate-x-full'} ${isMobileDevice ? 'shadow-2xl' : 'shadow-none'}`}>
      ...
      {/* Workspace Switcher & Mobile Close Button Area */}
      <div className="p-4 border-b border-slate-800/80 shrink-0">
        ...
        {isMobileDevice && (
          <div className="flex items-center justify-between mb-4 pl-1">
            ...
            <button onClick={() => setIsOpen(false)} className="p-1.5 bg-slate-800/80 hover:bg-red-900/30 rounded-full text-slate-400 hover:text-red-400 transition-colors shadow-sm">
              <X size={16} />
            </button>
          </div>
        )}
        ...
        <div className="relative" ref={switcherRef}>
          ...
          >
            ...
            {/* The Arrow is perfectly visible now */}
            <ChevronDown size={18} className={`text-slate-500 shrink-0 transition-transform duration-300 ${isSwitcherOpen ? 'rotate-180' : ''}`} />
          </button>
