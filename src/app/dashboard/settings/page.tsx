...
import {
  ...
  Settings2, Shield, Bell, Palette, Key, ChevronRight,
...
const SETTINGS_TABS: { id: SettingsTab; label: string; icon: React.ElementType; description: string }[] = [
  ...
  { id: 'workspace', label: 'Workspace', icon: Globe, description: 'Business settings' },
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 px-6 text-center">
      ...
      {description && (
        <p className="text-xs text-slate-400 dark:text-slate-600 mt-1 max-w-[220px] leading-relaxed">{description}</p>
      )}
    </div>
  );
}
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                disabled
                className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-medium text-slate-400 dark:text-slate-500 cursor-not-allowed"
              />
            </div>
            <div>
          ...
          <div className="flex items-center gap-2.5 p-3 bg-blue-50/60 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/40">
            ...
            <p className="text-[11px] text-blue-600 dark:text-blue-400 font-medium">
              Manage workspace settings from Brand Settings in the sidebar.
            </p>
          </div>
        </div>
      </div>
...
function SecurityTab() {
  const items = [
    { icon: Key, label: 'API Keys', description: 'Manage your API credentials', href: '/dashboard/api' },
    { icon: Fingerprint, label: 'Authentication', description: 'Two-factor & session management', href: '#' },
    { icon: Shield, label: 'Permissions', description: 'Role access control', href: '#' },
  ];
        ...
        {items.map(item => (
          <div key={item.label} className="flex items-center justify-between px-5 py-4 hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors cursor-pointer group">
            <div className="flex items-center gap-3.5">
              <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center group-hover:bg-slate-200 dark:group-hover:bg-slate-700/60 transition-colors">
            ...
            <ChevronRight size={15} className="text-slate-300 dark:text-slate-600 group-hover:text-slate-500 transition-colors" />
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden divide-y divide-slate-50 dark:divide-slate-800/60">
      {items.map(item => (
        <div key={item.label} className="flex items-center justify-between px-5 py-4">
          ...
          <div className={`w-10 h-5.5 rounded-full flex items-center transition-colors cursor-pointer px-0.5 ${item.enabled ? 'bg-blue-600 justify-end' : 'bg-slate-200 dark:bg-slate-700 justify-start'}`}>
    ...
    return (
      <div className="w-full max-w-4xl mx-auto space-y-6 pb-12">
        ...
        <div className="flex items-center justify-between animate-pulse">
          <div className="space-y-2">
            <div className="w-32 h-5 bg-slate-100 dark:bg-slate-800 rounded-lg" />
            <div className="w-48 h-3.5 bg-slate-100 dark:bg-slate-800 rounded-lg" />
          </div>
        ...
        <div className="flex gap-1 p-1 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-100 dark:border-slate-800 animate-pulse">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="flex-1 h-10 bg-slate-100 dark:bg-slate-800 rounded-xl" />
          ))}
        </div>
        {/* Card skeleton */}
        <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center gap-4 p-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 max-w-xs">
            Select a business from the sidebar to manage your team and settings.
          </p>
        </div>
      </div>
    );
    ...
    <div className="w-full max-w-4xl mx-auto space-y-5 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 pl-[23px]">
        ...
        {activeTab === 'team' && (
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-xs font-semibold rounded-xl transition-all shadow-sm shadow-blue-500/25 shrink-0"
          >
        ...
        {SETTINGS_TABS.map(tab => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1.5 px-2 py-2 sm:py-2.5 rounded-xl text-xs font-medium transition-all ${
                isActive
                  ? 'bg-white dark:bg-[#111827] text-slate-900 dark:text-white shadow-sm border border-slate-200 dark:border-slate-700'
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div
                key={stat.label}
                className="bg-white dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-slate-800 px-4 py-3.5"
              >
            ...
            {members.length === 0 ? (
              ...
              <div className="divide-y divide-slate-50 dark:divide-slate-800/50">
                {members.map(member => (
                  <div
                    key={member.id}
                    className="flex items-center justify-between px-5 py-3.5 hover:bg-slate-50/60 dark:hover:bg-slate-800/20 transition-colors group"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-mono font-semibold text-slate-700 dark:text-slate-300 truncate leading-tight">
                          {member.user_id}
                        </p>
                    ...
                    <div className="flex items-center gap-2.5 shrink-0">
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1.5 text-slate-300 dark:text-slate-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all"
                        title="Remove member"
                      >
          ...
          <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            <SectionHeader
              ...
              countVariant={invitations.length > 0 ? 'amber' : 'default'}
            />
              ...
              <div className="divide-y divide-slate-50 dark:divide-slate-800/50">
                {invitations.map(invite => {
                  ...
                  return (
                    <div
                      key={invite.id}
                      className="px-5 py-4 hover:bg-slate-50/60 dark:hover:bg-slate-800/20 transition-colors"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                              <span className="text-[10px] text-slate-400">
                                {new Date(invite.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
                              </span>
                              {invite.expires_at && (
                                <>
                        ...
                        <div className="flex items-center gap-2 shrink-0 pl-12 sm:pl-0">
                          ...
                          <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-semibold transition-all ${
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-semibold text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 border border-transparent hover:border-red-100 dark:hover:border-red-900 transition-all"
                          >
      ...
      {isModalOpen && (
        ...
        >
          <div
            className="w-full sm:max-w-md bg-white dark:bg-[#111827] sm:rounded-2xl rounded-t-3xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden"
          ...
          >
            ...
            <form onSubmit={handleSendInvite} className="p-5 space-y-4">
              ...
              >
                {sendingInvite
                  ? <><Loader2 size={13} className="animate-spin" /> Sending invitation…</>
                  : <><Plus size={13} strokeWidth={2.5} /> Send Invitation</>
                }
              </button>
            </form>
          </div>
