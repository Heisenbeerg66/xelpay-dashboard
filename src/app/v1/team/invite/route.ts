...
export async function POST(req: Request) {
  try {
    ...
    // ✅ FIX: businesses টেবিলে column নাম 'business_name', 'name' নয়
    ...
    // ✅ Create invitation — সব column নাম SQL schema-র সাথে মিলানো
    ...
    // ✅ FIX: businessData.business_name ব্যবহার করা হচ্ছে
    const workspaceName = businessData.business_name || SITE_NAME;
    ...
    if (sendError) {
      ...
      // Email send fail হলে invitation delete করো
...
export async function PATCH(req: Request) {
  try {
    ...
    // ✅ FIX: businesses টেবিলে 'business_name' column, 'name' নয়
    ...
    if (updateError) {
      console.error('Resend count update error:', updateError);
    }
    ...
    console.error('Resend invite PATCH error:', error);
    return NextResponse.json({ error: 'Server error.', details: error.message }, { status: 500 });
  }
}