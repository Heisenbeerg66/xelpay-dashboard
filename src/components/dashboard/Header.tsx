...
function useIsMobileDevice() {
  ...
  useEffect(() => {
    // Also consider small screens (md breakpoint) as mobile for responsive layout
...
export default function Header({ merchant, setSidebarOpen }: any) {
  ...
  return (
    <header className="sticky top-0 z-30 w-full bg-white/80 dark:bg-[#111827]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 md:px-8 py-3.5 flex items-center justify-between transition-colors duration-500">
      
      <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
        {/* Toggle Sidebar Button (Visible on mobile/tablet) */}
        <button 
          onClick={() => setSidebarOpen(true)} 
          className="md:hidden p-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-600 dark:text-slate-300 shrink-0"
        >
          <Menu size={20} />
        </button>
      ...
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4 shrink-0">
        ...
        <button onClick={toggleTheme} className="p-2 sm:p-2.5 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-xl text-slate-600 dark:text-slate-400 hover:text-blue-600 hover:border-blue-500 transition-all">
          {theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}
        </button>

        <button className="hidden sm:block relative p-2.5 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-xl text-slate-600 dark:text-slate-400 hover:text-blue-600 hover:border-blue-500 transition-all active:scale-95">
          <Bell size={18} />
        ...
        <div className="relative" ref={profileRef}>
          <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-2 p-1.5 sm:pr-3 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all active:scale-95">
            ...
            <ChevronDown size={14} className={`hidden sm:block text-slate-500 transition-transform ${isProfileOpen ? 'rotate-180' : ''}`} />
          </button>
