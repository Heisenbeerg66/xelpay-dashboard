...
import { Mail, ArrowLeft, Send, CheckCircle, Lock, Eye, EyeOff, Menu, X, Home, HelpCircle, Sun, Moon } from 'lucide-react';
...
function ForgotPasswordContent() {
  ...
  useEffect(() => {
    ...
    if (savedCooldownEnd) {
      ...
      const now = Date.now();
      if (endTime > now) {
        setCooldown(Math.floor((endTime - now) / 1000));
      }
    }
  ...
  const handleSendOtp = async (e?: React.FormEvent, isResend = false) => {
    ...
    setLoading(true);
    const { data: checkUser } = await supabase.from('merchants').select('is_demo').eq('email', email.trim()).maybeSingle();
    
    if (!checkUser) {
      toast.error("No account found with this email address.");
      setLoading(false);
      if (!isResend) recaptchaRef.current?.reset();
      setCaptchaToken(null);
      return;
    }

    if (checkUser.is_demo || mode === 'demo') {
      setTimeout(() => {
        setStep(4);
        toast.success("Demo Recovery successful!");
        setLoading(false);
      }, 1500);
      return;
    }
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!captchaToken) {
      toast.error("Please complete the reCAPTCHA verification to continue.");
      return;
    }
    if (!passwordRegex.test(newPassword)) {
      toast.error("Password must contain at least 8 characters, 1 uppercase letter, and 1 number.");
      return;
    }
  ...
  const generateStrongPassword = () => {
    ...
    let pw = uppers[Math.floor(Math.random() * uppers.length)] + 
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B1120] flex flex-col font-sans transition-colors duration-300">
      ...
      {/* ── MAIN CONTENT (Mobile top aligned) ── */}
          ...
          {step === 2 && (
            <div className="animate-in slide-in-from-right-8 duration-500">
              <button 
                ...
                className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-blue-600 mb-5 transition-colors uppercase tracking-widest"
              >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="• • • • • •" 
                    disabled={loading}
                    className={`${inputClass} !pl-4 text-center tracking-[1em] text-2xl font-bold disabled:opacity-50`} 
                ...
                <div className="text-center pt-2">
                  <button
                    ...
                    className="text-xs font-medium text-blue-600 hover:underline disabled:text-slate-400 disabled:no-underline transition-all"
                  >
                    {resendCount >= 3
              ...
              <form onSubmit={handleUpdatePassword} className="space-y-5">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-2 leading-relaxed">
                    Must be at least 8 characters, containing 1 uppercase and 1 number. (Symbols are optional)
                  </p>
                </div>
                ...
                <div className="pt-2 text-center">
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <Link href="/login" onClick={clearSession} className="text-blue-600 font-semibold hover:underline">Login Now</Link>
                  </p>
                </div>
              </form>
            </div>
