...
// Added Eye, EyeOff for masking toggle
import { Smartphone, Download, Battery, Cpu, Copy, Loader2, X, RefreshCw, Trash2, Replace, Key, QrCode, DownloadCloud, CheckCircle2, Eye, EyeOff, ArrowLeft } from 'lucide-react';
...
export default function BusinessDevicesPage() {
    ...
    return (
        <div className="w-full space-y-6 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
            ...
            {/* Header - Stacks nicely on mobile, row on desktop */}
            <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-5 border-b border-slate-200 dark:border-slate-800 pb-6 relative z-10">
                <div className="flex items-center gap-3 justify-center sm:justify-start">
                    ...
                    <div className="text-center sm:text-left">
                        ...
                        <p className="text-slate-500 font-bold text-xs md:text-sm mt-1 uppercase tracking-wider">Workspace SMS Node</p>
                    </div>
                </div>
                
                <div className="flex flex-row gap-3 w-full sm:w-auto">
                    <button onClick={handleOpenImportModal} disabled={importing} className="flex-1 sm:flex-none bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-white px-5 py-3 rounded-xl font-bold flex items-center justify-center gap-2 text-xs md:text-sm uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 shadow-sm">
                    ...
                    <button onClick={() => setIsDownloadModalOpen(true)} className="flex-1 sm:flex-none bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3 rounded-xl font-bold flex items-center justify-center gap-2 text-xs md:text-sm uppercase tracking-widest transition-all shadow-md active:scale-95">
                ...
                <div className="w-full max-w-2xl relative z-10">
                    {!activeDevice ? (
                        <div className="bg-white dark:bg-[#111827] rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 p-8 md:p-12 flex flex-col items-center text-center shadow-sm">
                            ...
                            <p className="text-sm text-slate-600 dark:text-slate-400 font-medium mb-10 leading-relaxed max-w-md">Install the app and connect it to your workspace to automate SMS parsing. Only one node can be active per workspace.</p>
                            
                            {/* Actions stack nicely on premium mobile view */}
                            <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
                                <button onClick={handleAutoConnect} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-md active:scale-95 transition-all flex items-center justify-center gap-2">
                        ...
                        <div className="bg-white dark:bg-[#111827] rounded-3xl border border-slate-100 dark:border-slate-800 border-b-[4px] border-b-indigo-500 shadow-xl p-6 md:p-10 relative overflow-hidden">
                            ...
                            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 mb-8 relative z-10">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-xl sm:text-2xl font-black text-slate-950 dark:text-white tracking-tight leading-tight">{activeDevice.device_name || 'Business Gateway'}</h3>
            ...
            {/* IMPORT MODAL - Kept structural logic */}
            {isImportModalOpen && vaultData && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setIsImportModalOpen(false)}>
                    <div onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-[#111827] w-full max-w-md rounded-2xl shadow-2xl animate-in zoom-in-95 overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                            <div>
                                <h2 className="text-base font-black text-slate-950 dark:text-white uppercase tracking-tight">Import Device</h2>
                        ...
                        <div className="p-8 flex flex-col gap-4 bg-white dark:bg-[#0B1120]">
                            <div className="bg-slate-50 dark:bg-[#111827] p-6 rounded-2xl border border-slate-100 dark:border-slate-800 text-center">
                                ...
                                <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Vault Master Device</p>
                                <p className="text-lg font-black text-slate-950 dark:text-white">{vaultData.device_name || 'Unknown Device'}</p>
                                <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">Model: {vaultData.device_model || 'N/A'}</p>
                            </div>
                            <button onClick={handleConfirmImport} disabled={importing} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs uppercase tracking-widest py-4 rounded-2xl shadow-xl active:scale-95 transition-all flex justify-center items-center gap-2.5 disabled:opacity-50 mt-2">
            ...
            {/* DOWNLOAD MODAL - improved styling for premium view */}
            {isDownloadModalOpen && (
            ...
            {/* MANUAL PAIR MODAL - Fixed plain Simple Square QR and Masked key */}
            {isManualModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setIsManualModalOpen(false)}>
                    <div onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-[#111827] w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col">
                        ...
                        <div className="p-8 bg-white dark:bg-[#0B1120] flex flex-col items-center text-center">
                            {/* FIXED Simple Square QR, removed green corners absolute positioned */}
                            <div className="bg-white rounded-2xl p-6 shadow-inner border border-slate-100 mb-8 relative">
                                <QRCodeSVG value={businessData?.device_connection_key || 'generating...'} size={180} level="H" fgColor="#0F172A" bgColor="#ffffff" />
                            </div>
