...
import { Building2, Globe, Smartphone, ShoppingBag, LayoutGrid, Mail, Phone, DollarSign, BadgeDollarSign, ShieldCheck, Copy, Loader2, AlertTriangle, CheckCircle2 } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import { toast, Toaster } from 'sonner';
import { autoVerifyBusiness, checkDuplicateWebsite, verifyDomain, verifyDomainOwnership } from '@/lib/verify-business';

// ── Inline utility — do NOT import this from a 'use server' file ──────────────
...
export default function NewBusiness() {
  ...
  const [duplicateInfo, setDuplicateInfo] = useState<{ existingBusinessId?: string } | null>(null);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isLimitReached) { toast.error(`Your plan allows max ${businessLimit} business(es). Please upgrade.`); return; }
    setLoading(true);
    setErrorMessage('');
    try {
      ...
      if (needsDuplicateCheck && formData.websiteUrl) {
        ...
        if (dupeResult.isDuplicate) {
          setDuplicateInfo({ existingBusinessId: dupeResult.existingBusinessId });
      ...
      const errMsg = error.message || 'Failed to create workspace.';
      toast.error(errMsg);
  ...
  const createPendingBusiness = async (): Promise<string> => {
    const { data: { user } } = await supabase.auth.getUser();
    ...
    const code = verifyCode || generateVerifyCode();
  ...
  const handleVerifyAndSubmit = async () => {
    ...
    try {
      ...
      if (!bizId) { bizId = await createPendingBusiness(); setPendingBusinessId(bizId); }
  ...
  const handleOwnershipVerify = async () => {
    ...
    try {
      ...
      if (result.success) {
        autoVerifyBusiness(bizId, formData.websiteUrl);
        toast.success('Ownership verified! Business submitted.');
        window.dispatchEvent(new Event('businessChanged'));
        setTimeout(() => { window.location.href = '/dashboard'; }, 1500);
      } else {
        toast.error(result.message);
  ...
  const submitBusiness = async () => {
    ...
    setErrorMessage('');
    window.dispatchEvent(new Event('businessChanged'));
    if (formData.businessType === 'F Commerce' && formData.websiteUrl) {
  ...
  if (step === 'domain_verify') {
    ...
    return (
      <div className="max-w-2xl mx-auto pb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="mb-6">
          <h1 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white">Verify Your Website</h1>
          <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">Prove ownership of <span className="font-semibold text-blue-600">{formData.websiteUrl}</span></p>
        </div>
          ...
          <div className="p-6 space-y-6">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">Add this tag inside <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded text-xs">&lt;head&gt;</code>:</p>
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">Publish changes, clear cache, then click Verify below.</p>
            </div>
            <div className="p-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-xl text-xs text-amber-700 dark:text-amber-400 font-medium">
              ⚠️ Make sure the meta tag is publicly accessible and your website is live.
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={handleVerifyAndSubmit} disabled={verifying} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 disabled:opacity-60">
                {verifying ? <><Loader2 size={16} className="animate-spin" /> Verifying...</> : <><ShieldCheck size={16} /> Verify & Submit</>}
              </button>
              <button onClick={() => setStep('form')} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 py-3.5 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
                ← Back
              </button>
            </div>
          </div>
      ...
      <div className="max-w-2xl mx-auto pb-8 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
          <div className="bg-gradient-to-br from-amber-500 to-orange-600 px-6 py-8 text-white text-center">
            ...
            <p className="text-amber-100 text-sm mt-1.5">This domain is already associated with an existing account.</p>
          </div>
          <div className="p-6 space-y-5">
            <div className="bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl p-4 text-sm text-slate-600 dark:text-slate-400">
              <p>The domain <span className="font-semibold text-slate-900 dark:text-white">{formData.websiteUrl}</span> is already registered. If you own it, you can verify ownership to proceed.</p>
            </div>
            <div className="space-y-3">
              <button
                onClick={() => { const code = generateVerifyCode(); setVerifyCode(code); setStep('ownership_verify'); }}
  ...
  if (step === 'ownership_verify') {
    ...
    return (
      <div className="max-w-2xl mx-auto pb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <Toaster position="top-center" richColors />
        <div className="mb-6">
          <h1 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white">Prove Domain Ownership</h1>
          <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">Verify <span className="font-semibold text-blue-600">{formData.websiteUrl}</span></p>
        </div>
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-lg">
          <div className="flex border-b border-slate-200 dark:border-slate-800">
            {(['meta', 'txt'] as const).map((m) => (
              <button key={m} onClick={() => setOwnershipMethod(m)}
          ...
          <div className="p-6 space-y-5">
            {ownershipMethod === 'meta' ? (
              <div className="space-y-3">
                <p className="text-sm text-slate-600 dark:text-slate-400">Add this inside <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded">&lt;head&gt;</code>:</p>
                <div className="bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl p-4 font-mono text-xs text-slate-700 dark:text-slate-300 break-all flex items-start justify-between gap-3">
                  <span>{metaTag}</span>
                  <button onClick={() => copyToClipboard(metaTag)} className="shrink-0 p-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg hover:text-blue-600"><Copy size={14} /></button>
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-sm text-slate-600 dark:text-slate-400">Add a DNS <strong>TXT record</strong>:</p>
                <div className="bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl p-4 text-xs font-mono space-y-2">
                  <div className="flex justify-between"><span className="text-slate-500">Type:</span><span className="font-bold text-slate-900 dark:text-white">TXT</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">Name:</span><span className="font-bold text-slate-900 dark:text-white">@</span></div>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 min-w-0"><span className="text-slate-500">Value: </span><span className="text-blue-600 font-bold break-all">{txtRecord}</span></div>
                ...
                <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-lg p-2">⏱️ DNS changes may take up to 48 hours to propagate.</p>
              </div>
            )}
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={handleOwnershipVerify} disabled={verifying} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 disabled:opacity-60">
                {verifying ? <><Loader2 size={16} className="animate-spin" /> Verifying...</> : <><CheckCircle2 size={16} /> Verify Ownership</>}
              </button>
              <button onClick={() => { setStep('form'); setDuplicateInfo(null); setPendingBusinessId(null); }} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 py-3.5 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
                ← Cancel
              </button>
            </div>
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">Fill in your business details to get started with XelPay.</p>
      </div>
      ...
      {isLimitReached && (
        <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/30 rounded-xl flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 dark:text-red-500 mt-0.5">Your plan allows max {businessLimit} business workspace(s). Please upgrade.</p>
          </div>
        </div>
      )}

      {errorMessage && (
        <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/30 rounded-xl text-xs text-red-600 dark:text-red-400 font-medium">{errorMessage}</div>
      )}
      ...
      <form onSubmit={handleFormSubmit} className="space-y-5">
        ...
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">Business Name <span className="text-red-500">*</span></label>
              <div className="relative group">
                ...
                <input required type="text" placeholder="Easy Earn" value={formData.businessName}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">Business Type <span className="text-red-500">*</span></label>
              <div className="relative group">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">{linkConfig.label} <span className="text-red-500">*</span></label>
            <div className="relative group">
              ...
              <input required type="url" placeholder={linkConfig.placeholder} value={formData.websiteUrl}
                onChange={(e) => setFormData({ ...formData, websiteUrl: e.target.value })}
                className="w-full pl-9 pr-3 py-2.5 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600/20 text-sm font-medium text-slate-900 dark:text-white transition-all placeholder:text-slate-400" />
            </div>
            {formData.businessType === 'Website' && (
              <p className="text-[11px] text-blue-600 dark:text-blue-400 font-medium ml-1">🔒 Website ownership verification required before submission.</p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                ...
                <input type="email" placeholder="support@mybusiness.com" value={formData.supportEmail}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                ...
                <input type="tel" placeholder="+880 1700 000000" value={formData.supportPhone}
        ...
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">Currency <span className="text-red-500">*</span></label>
              <div className="relative group">
                ...
                <select required value={formData.currency} onChange={(e) => setFormData({ ...formData, currency: e.target.value })}
                  className="w-full pl-9 pr-3 py-2.5 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600/20 text-sm font-medium text-slate-900 dark:text-white transition-all appearance-none">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">USD → BDT Rate <span className="text-red-500">*</span></label>
              <div className="relative group">
                ...
                <input required type="number" step="0.01" min="1" placeholder="e.g. 110" value={formData.exchangeRate}
