...
export default function TeamSettingsPage() {
  ...
  // ✅ FIX 1: devices/page.tsx এর exact same pattern — useCallback বাদ দিয়ে
  // সরাসরি useEffect এর ভেতরে ফাংশন define করা, empty deps []
  // এতে page load এ race condition হয় না
  useEffect(() => {
    const loadActiveBusiness = () => {
      ...
      if (activeId) {
        ...
        setLoading(false); // কোনো business নেই
      }
    };
  ...
  }, []); // ✅ empty deps — একবারই mount, businessId dependency নেই
  ...
  // ✅ FIX 2: সঠিক API URL — /api/v1/team/invite
  // আগে ভুল ছিল: /api/business/team/invite (এই route exist করে না)
  const handleSendInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ content-type চেক — HTML page আসলে JSON parse এ crash করবে না
  ...
  const getRoleBadge = (role: Role) => {
    ...
    return (
      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black border ${styles[role]}`}>
        {role}
      </span>
    );
  };
  ...
  if (loading) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center">
      ...
      <p className="text-sm font-bold text-slate-500 uppercase">Loading Team</p>
    </div>
  );
    ...
    <div className="w-full space-y-8 pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        ...
        <button
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-xl font-black flex items-center justify-center gap-2 text-xs uppercase tracking-widest shrink-0 transition-colors"
        >
        ...
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
          ...
          {members.length === 0 ? (
            <div className="py-12 text-center text-slate-400">
              ...
              <p className="text-sm font-bold">No active members yet.</p>
              <p className="text-xs mt-1">Invite colleagues to collaborate.</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {members.map(member => (
                <div key={member.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center font-black text-sm">
                      {(member.user_email || member.user_id || 'U')[0].toUpperCase()}
                    </div>
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 uppercase">
                        Joined: {new Date(member.added_at || member.created_at || '').toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                ...
                <div key={invite.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button
                      ...
                      className="text-[11px] font-black text-slate-500 hover:text-red-600 px-3 py-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg uppercase transition-colors"
                    >
                      Revoke
                    </button>
                  </div>
          ...
          <div className="w-full max-w-md bg-white dark:bg-[#111827] rounded-[28px] overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <span className="text-base font-black text-slate-900 dark:text-white uppercase">Send Invitation</span>
              </div>
              <button
                onClick={() => { setIsInviteModalOpen(false); setInviteEmail(''); setInviteRole('viewer'); }}
                className="p-2 bg-slate-100 dark:bg-slate-800 text-slate-400 rounded-xl hover:text-slate-600 transition-colors"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleSendInvite} className="p-6 space-y-5">
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase block mb-1.5">Email Address</label>
                <input
                  type="email"
                  required
                  value={inviteEmail}
                  ...
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-medium text-slate-900 dark:text-white focus:border-blue-500 focus:outline-none transition-colors placeholder:text-slate-400"
                />
              </div>

              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase block mb-1.5">Assign Role</label>
                <select
                  value={inviteRole}
                  onChange={e => setInviteRole(e.target.value as Role)}
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-medium text-slate-900 dark:text-white focus:border-blue-500 focus:outline-none transition-colors"
                >
              ...
              <div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/30">
                ...
                <p className="text-[11px] text-blue-600 dark:text-blue-400">
                  An invite link will be sent to their email. Link is valid for 7 days.
                </p>
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-black text-sm uppercase rounded-xl flex items-center justify-center gap-2 transition-all"
              >
