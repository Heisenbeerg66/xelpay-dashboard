...
import { Mail, Lock, User, MapPin, CheckCircle, ArrowRight, X, AlertCircle, LogIn, Users, Loader2, EyeOff, Eye, Check, Moon, Sun } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B1120] font-sans transition-colors duration-300">
      ...
      <header className="sticky top-0 z-30 bg-white/90 dark:bg-[#0B1120]/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 h-16 flex items-center px-4 md:px-8 justify-between">
        <div /> {/* left spacer — no branding per request */}
        <div className="flex items-center gap-2">
          ...
          <Link href="/login"
            className="w-9 h-9 flex items-center justify-center rounded-full bg-blue-600 text-white hover:bg-blue-700 transition-all">
            <LogIn size={16} />
          </Link>
        </div>
      </header>

      {/* Fix 9: Mobile — full scroll (no sticky cart) */}
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-12 md:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start">

          {/* Left: Plan Cart — Fix 9: not sticky on mobile */}
            ...
            <form onSubmit={handleSignUp} className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
              ...
              <div className="md:col-span-2">
                ...
                <div className="relative">
                  ...
                  <input required type="email" className={`${inputClass} pl-10`} placeholder="rahim@gmail.com"
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden focus-within:ring-1 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all">
                  <select value={countryCode} onChange={e => setCountryCode(e.target.value)}
                    className="bg-transparent border-r border-slate-200 dark:border-slate-700 px-3 py-3.5 outline-none text-sm font-medium text-slate-700 dark:text-slate-300 shrink-0">
                    <option value="+880">🇧🇩 +880</option>
                    <option value="+1">🇺🇸 +1</option>
                    <option value="+44">🇬🇧 +44</option>
                    <option value="+91">🇮🇳 +91</option>
                  </select>
                  <input required type="tel" className="w-full px-4 py-3.5 bg-transparent outline-none text-sm text-slate-900 dark:text-white placeholder:text-slate-400"
              ...
              <div>
                ...
                <div className="flex justify-between items-center px-0.5 mt-1.5">
                  <span className={`text-[10px] ${formData.password.length >= 8 && /[A-Z]/.test(formData.password) && /[0-9]/.test(formData.password) ? 'text-green-500' : 'text-slate-400'}`}>
                    Min 8 chars, 1 uppercase, 1 number
                  </span>
                  <button type="button" onClick={() => {
                    const chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#";
                    let gen = "Xp9!" + Array.from({ length: 10 }, () => chars[Math.floor(Math.random() * chars.length)]).join('');
              ...
              <div className="md:col-span-2">
                <label className="flex items-start gap-3 cursor-pointer select-none">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                    I agree to the{' '}
                    <a href={privacyPolicyLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline" onClick={e => e.stopPropagation()}>Terms & Conditions</a>
                    {' '}and{' '}
