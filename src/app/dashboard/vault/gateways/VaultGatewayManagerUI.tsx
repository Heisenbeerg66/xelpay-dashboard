...
export default function VaultGatewayManagerUI({ merchantId }: { merchantId: string }) {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (res.success) {
            ...
            setGateways(
                editingId
                    ? gateways.map((g) => (g.id === editingId ? res.data : g))
                    : [res.data, ...gateways]
            );
            ...
            <div className="flex flex-row items-center justify-between gap-3 mb-8">
                <div className="flex-1 min-w-0 flex items-center gap-3">
                    <button
                        ...
                        className="p-2 -ml-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-all text-slate-500"
                    >
                        <ArrowLeft size={24} />
                    </button>
                    <div>
                        ...
                        <p className="text-[11px] md:text-sm text-slate-500 dark:text-slate-400 mt-0.5 md:mt-1.5 font-medium truncate">
                            Manage your vault receiving methods.
                        </p>
                    </div>
                </div>
                <button
                ...
                {gateways.length === 0 && (
                    <div className="col-span-full py-24 flex flex-col items-center justify-center text-center opacity-60 bg-white dark:bg-[#111827] rounded-[30px] border border-slate-100 dark:border-slate-800 border-dashed">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm font-medium">
                            You haven't added any vault gateways yet. Click the button above to get started.
                        </p>
                    </div>
                )}
            </div>
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar bg-white dark:bg-[#111827]">
                            <form id="vaultGatewayForm" onSubmit={handleSave} className="space-y-6">
                                ...
                                {provider && (
                                    <div className="space-y-6">
                                        <div className="grid grid-cols-2 gap-4 pt-6 mt-6 border-t border-slate-100 dark:border-slate-800">
                                            <div>
                                                <label className={labelClass}>
                                                    Min ({activeTab === 'international' ? 'USD' : 'BDT'}) {reqStar}
                                                </label>
                                                <input
                                                    required
                                                    name="min_amount"
                                            ...
                                            <div>
                                                <label className={labelClass}>
                                                    Max ({activeTab === 'international' ? 'USD' : 'BDT'}){' '}
                            ...
                            >
                                {isSaving ? (
                                    ...
                                    <>
                                        ...
                                        {editingId ? 'Update Vault Gateway' : 'Securely Save Gateway'}
                                    </>
                                )}
                            </button>
                        </div>
